(function(win, doc) {
    ...
    let username = prompt("Qual o seu nome") || 'Anon';
    ...
    /*
      ...
      `$inputUsername`.
      */
    const $inputUsername = document.querySelector('[type="text"]');
    /*
    ...
    $button.addEventListener("click", event => {
     // var hasBeenValid = isValidEmail($inputEmail.value);
      const arr = [$inputEmail, $inputEmail, $message];
      var hasVoidQuery = arr.some(item => !item.value);

      isValidEmail($inputEmail.value);
      var hasEmailValid = isValidEmail($inputEmail.value);

    
      if(hasEmailValid && !hasVoidQuery) {
        let test = confirm("Tem certeza que deseja enviar o formulário?") ? alert('Enviado') : alert('Não enviado');
        if(!test){
          return false;
        }
