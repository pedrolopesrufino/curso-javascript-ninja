(function() {
  ...
  const text =
    ...
    "apelidado de O Centauro de Luvas, foi um militar, " +
  ...
  */
  ...
  console.log(text.replace(/Manuel Marques de Sousa/, "Pedro Lopes Rufino"));
  ...
  */
  ...
  console.log(
    text.replace(/O Centauro de Luvas/, string => string.toUpperCase())
  );
  
  /*
  Agora iremos substituir as datas no formato 13 de junho de 1804 para
  13061804. A primeira coisa a fazer é criar uma função chamada
  ...
  const getMonthNumber = monthName => {
    ...
    monthName = (months.indexOf(monthName) + 1).toString();
  
    return monthName.length === 1 ? "0" + monthName : monthName;
  };
  
  console.log(
    "o mês março é representado pelo número " + getMonthNumber("março")
  );
  console.log(
    "o mês setembro é representado pelo número " + getMonthNumber("setembro")
  );
  console.log(
    "o mês dezembro é representado pelo número " + getMonthNumber("dezembro")
  );
  ...
  var regexDay = text.match(/ \d. /g);
  let regexMonth = text.match(/ju[a-z]{3}/g);
  var regexYear = text.match(/\d{4}/g);
  ...
  console.log("\nReplace de datas");
  const replaceDate = (day, month, year) => {
    month = getMonthNumber(month);
  
    return console.log(day, `/`, month, `/`, year);
  };
  console.log("hey", regexMonth[0]);
  
  replaceDate(regexDay[0], regexMonth[0], regexYear[0]);
  replaceDate(regexDay[1], regexMonth[1], regexYear[1]);
