(function(){
  ...
  */
  ...
  var regex = /container/gm; 

  const hasClass = (markup,cssClass) =>  {
    markup.replace(regex, function(regex){
      
    })
    
    
    return ` ${(cssClass == markup.match(regex))} para a classe ${cssClass}` ;
  ...
  console.log(hasClass(markup,'main'))
      //return console.log(`para a classe ${cssClass} o resultado é ${markup.search(regResult) ? true : false}`);
